...
static void *_server(void *arg)
{
	...
	{
		int err;
		EXOS_TREE_DEVICE *dev_node = (EXOS_TREE_DEVICE *)exos_tree_find_path(NULL, "dev/iap/com.miolabs.iap2test");
		if (dev_node != NULL)
		{
			...
			if (err == 0)
			{
				...
				if (dev_node == NULL)
					dev_node = (EXOS_TREE_DEVICE *)exos_tree_find_path(NULL, "dev/usbftdi1");
