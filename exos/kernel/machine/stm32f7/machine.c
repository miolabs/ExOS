...
void __machine_init()
{
	...
	ASSERT(main_clk <= 210000000UL, KERNEL_ERROR_KERNEL_PANIC);
	unsigned usb_clk = vco_out / PLLQ_VALUE;
	ASSERT(usb_clk <= 48000000UL, KERNEL_ERROR_KERNEL_PANIC);

	//NOTE: latency selection for STM32F76xxx 77xxx
	unsigned latency = 0;
	if (main_clk > 210000000UL) latency = 7;
	else if (main_clk > 180000000UL) latency = 6;
	else if (main_clk > 150000000UL) latency = 5;
	...
	// TODO: enable ART accelerator?

	if (latency != 0)
		FLASH->ACR |= FLASH_ACR_PRFTEN;	// enable flash prefetch buffer

	unsigned ppre1 = 0;
	for(unsigned apb1 = main_clk; apb1 >= 54000000UL; apb1 >>= 1) ppre1++;
	unsigned ppre2 = 0;
	for(unsigned apb2 = main_clk; apb2 >= 108000000UL; apb2 >>= 1) ppre2++;
