...
static unsigned _enum_interfaces(usb_host_device_t *device, usb_configuration_descriptor_t *conf_desc)
{
	...
	while (offset < USB16TOH(conf_desc->TotalLength))
	{
		...
		else if (fn_desc->DescriptorType == USB_DESCRIPTOR_TYPE_INTERFACE_ASSOCIATION)
		{
			...
			_verbose(VERBOSE_DEBUG, "checking conf #%d, if asoc #%d (%d if)...", 
		...
		if (found_driver)
		{
			if (done == 0)
			{
				...
				{
					_verbose(VERBOSE_ERROR, "set configuration #%d failed!", conf_desc->ConfigurationIndex);
					break;
				}
			}
...
static bool _parse_configuration(usb_host_device_t *device, unsigned char conf_index)
{
	...
	if (done)
	{
		...
		_verbose(VERBOSE_DEBUG, "checking conf[%d] (#%d, val=%02xh)...", conf_index,
		...
		if (total_length <= USB_ENUM_BUFFER_SIZE) 
		{
			...
			if (done)
			{
				...
				if (!done) _verbose(VERBOSE_DEBUG, "no driver found for conf[%d]", conf_index);
			}
			else _verbose(VERBOSE_ERROR, "cannot read conf[%d] descriptor (full)", conf_index); 
		}
		else _verbose(VERBOSE_ERROR, "conf[%d] descriptor is too big!", conf_index);
	}
	else _verbose(VERBOSE_ERROR, "cannot read conf[%d] descriptor (short)", conf_index);
...
bool usb_host_enumerate(usb_host_device_t *device, usb_device_descriptor_t *dev_desc)
{
	...
	_verbose(VERBOSE_COMMENT, "device %04x:%04x", device->Vendor, device->Product);
