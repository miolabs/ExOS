...
static usb_otg_crs_host_t * const otg_host = (usb_otg_crs_host_t *)(USB_OTG_FS_BASE + 0x400);
...
static volatile uint32_t * const otg_fifo[NUM_CHANNELS] = {
	 ...
	 (volatile uint32_t *)(USB_OTG_FS_BASE + 0x7000), (volatile uint32_t *)(USB_OTG_FS_BASE + 0x8000) }; 

// NOTE: total words for OTG_FS is 320 (1280 bytes)
#define RX_WORDS 128
	...
	switch(_port_state)
	{
		...
		case HPORT_RESET:
			...
			else
			{
				...
				verbose(VERBOSE_DEBUG, "usb-fs-roothub", "device disconnected during reset!");
		...
		case HPORT_RESET_DONE:
			...
			break;
		case HPORT_ERROR:
			verbose(VERBOSE_DEBUG, "usb-fs-roothub", "error state!");
...
void __usb_otg_rxflvl_irq_handler()
{
	otg_global->GINTMSK &= ~USB_OTG_GINTMSK_RXFLVLM;
...
void __usb_otg_nptxfe_irq_handler()
{
...
void __usb_otg_hcint_irq_handler()
{
	while(1)
	{
...
void __usb_otg_host_irq_handler()
{
	...
	{
		__usb_otg_nptxfe_irq_handler();
	}

	if (sta & USB_OTG_GINTSTS_HCINT)
	{
		__usb_otg_hcint_irq_handler();
	}
