...
bool usb_hs_host_start_pipe(usb_host_pipe_t *pipe)
{
	...
	if (ep != nullptr)
	{
		...
		verbose(VERBOSE_DEBUG, "usb-hs-core", "alloc channel failed (addr=%d, ep=%d)", pipe->Device->Address, pipe->EndpointNumber); 
		pipe->Endpoint = nullptr;
		pool_free(&_ep_pool, &ep->Node);
	}
	else verbose(VERBOSE_DEBUG, "usb-hs-core", "alloc endpoint failed (addr=%d, ep=%d)", pipe->Device->Address, pipe->EndpointNumber); 
