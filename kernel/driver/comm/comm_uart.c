// comm driver for built-in uarts
// by Miguel Fides
...
static COMM_DEVICE _comm_device = { .Driver = &_comm_driver, .PortCount = UART_MODULE_COUNT };
static EXOS_TREE_DEVICE _devices[] = {
	{ .Name = "comm0", .Device = &_comm_device, .Unit = 0 },
#if UART_MODULE_COUNT > 1
	{ .Name = "comm1", .Device = &_comm_device, .Unit = 1 },
#endif
#if UART_MODULE_COUNT > 2
	{ .Name = "comm2", .Device = &_comm_device, .Unit = 2 },
#endif
#if UART_MODULE_COUNT > 3
