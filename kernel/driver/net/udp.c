...
int net_udp_send(NET_ADAPTER *adapter, const IP_ENDPOINT *destination, unsigned short source_port, unsigned short dest_port, NET_MBUF *data)
{
...
unsigned short net_udp_checksum(const IP_ADDR *source_ip, const IP_ADDR *dest_ip, NET_MBUF *mbuf, int offset)
{
	int total_length = 0;
	unsigned long sum = 0;
	while(mbuf != NULL)
